										...
										if _, cek := data.Guidelines[to]; cek {
											...
											for _, r := range data.Replace {
												if strings.Contains(text, r) {
													fmt.Println("replaceMod ",replaceMod)
													replaceMod = true
												}
											...
											if replaceMod == true {
												for _, v := range data.TOKENNOTIFY {
													for _, cont := range data.Content {
														c.Notify(context.Background(), v, cont, "", "", nil)
													}
												}
												replaceMod = false
											} else {
								...
								for TX := range box {
									...
									case "help2":
										if getAccess(ctime, cl.Mid) {
											...
											tx += "┃-กันเฟค เปิด/ปิด\n"
											tx += "┃-กันอัลบั้ม เปิด/ปิด\n"
											tx += "┃-กันสติ๊กเกอร์ เปิด/ปิด\n"
											tx += "┃-กันโทรกลุ่ม เปิด/ปิด\n"
											tx += "┃-กันส่งไฟล์ เปิด/ปิด\n"
											tx += "┃-กันโพส เปิด/ปิด\n"
											tx += "┃-กันส่งวีดีโอ เปิด/ปิด\n"
											tx += "┃-กันส่งคลิปเสียง เปิด/ปิด\n"
									...
									case "help3":
										if getAccess(ctime, cl.Mid) {
											tx := "┏🟢คำสั่งแนวทางกันกลุ่ม🟢\n"
											tx += "┃-กลุ่ม\n"
											tx += "┃-ส่งแนวทางเปิด\n"
											tx += "┃-ส่งแนวทางปิด\n"
											tx += "┃-ดึงแนวทางกลุ่ม #ลำดับกลุ่ม\n"
											tx += "┃-ยกเลิกดึงแนวทางกลุ่ม #ลำดับกลุ่ม\n"
											tx += "┃-tokennotify #ใส่ดทเค่น\n"
											tx += "┃-showtokennotify #ใส่ดทเค่น\n"
											tx += "┃-content\n"
											tx += "┃-addcontent #ใส่ content\n"
											tx += "┃-removecontent\n"
											tx += "┃-replace\n"
											tx += "┃-addreplace\n"
											tx += "┃-removereplace\n"
									...
									} else if strings.HasPrefix(txt, "addcontent ") {
										if getAccess(ctime, cl.Mid) {
											// result := strings.Split((text), " ")
											// s :=
											data.Content = append(data.Content, strings.Replace(text, "Addcontent ", "", -1))
											// data.url =

											cl.SendMessage(to, "เพิ่ม content สำเร็จ")
											SaveData()
										}
									} else if strings.HasPrefix(txt, "addreplace ") {
										if getAccess(ctime, cl.Mid) {
											// result := strings.Split((text), " ")
											// s :=
											data.Replace = append(data.Replace, strings.Replace(text, "Addreplace ", "", -1))
											// data.url =

											cl.SendMessage(to, "เพิ่ม replace สำเร็จ")
											SaveData()
										}
